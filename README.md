...
Чтобы объединить данные из четырех независимых источников без искажений и дублей, применены следующие инженерные решения:
...
* **Предварительная агрегация в CTE:** Каждая из 4 таблиц предварительно агрегируется по зерну в отдельных CTE-блоках. Это исключает «эффект веера» (fan-out effect) и завышение денежных метрик при соединении.
